...
def search_venues():
  ...
  flash(search_query)
  venues = Venue.query.filer(Venue.name.ilike('%' + search_query + '%')).all()  
  flash(search_query)
  venue_list = []
  for venue in venues:
...
def create_venue_submission():
  # TODO: insert form data as a new 
  ...
  try:
    ...
    error=True
    print(sys.exc_info)
    flash('error has occured. Venue ' + request.form['name'] + 'Could not be listed.')
  finally:
    db.session.close()
    if error == True:
      abort(400)
...
def search_artists():
  ...
  artist_list = Artist.query.filter(Artist.name.ilike('%' + search_query + '%')).order_by(Artist.name).all()
...
def create_artist_submission():
  ...
  try:
    ...
    error=True
    print(sys.exc_info)
    flash('error has occured. Artist ' + request.form['name'] + 'Could not be listed.')
  finally:
    db.session.close()
    if error == True:
      abort(400)
...
def create_show_submission():
  ...
  try:
    ...
    error=True
    print(sys.exc_info)
    flash('error has occured. Show ' + request.form['name'] + 'Could not be listed.')
  finally:
    db.session.close()
    if error == True:
      abort(400)
